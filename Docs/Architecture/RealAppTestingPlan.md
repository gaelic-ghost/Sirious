...
- Decision: the external user LaunchAgent is the active local helper lane. The bundled `SMAppService.agent(plistName:)` path remains a documented validation probe, but it is not the user-facing install path because current local validation still reports `.notFound` for package-style installed copies while the external LaunchAgent path reaches the non-sandboxed helper over XPC.
