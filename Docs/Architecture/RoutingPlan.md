...
3. `StreamingRouteClassifier` turns each transcript event into a route hypothesis.
4. `RiskAndContextGate` separates executable local decisions from actions that need confirmation or more context.
5. Route-specific executors handle app control, search, retrieval, planning, or clarification.
